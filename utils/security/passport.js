...
const utils = require("..");
var log_file = fs.createWriteStream(__dirname + "/../../log/access.log", {
  flags: "a",
});
...
module.exports = function (passport) {
  ...
  passport.use(
    ...
    new LocalStrategy(
      ...
      function (req, userid, password, done) {
        ...
        models.user
          .findOne({
            where: Sequelize.or({ logon_id: userid }, { email: userid }),
          })
          .then(function (user) {
            ...
            if (!user) {
              ...
              return done(
                null,
                false,
                req.flash("loginMessage", "Benutzer nicht gefunden")
  ...
  if (settings.config.get("auth.user.method").includes("oidc")) {
    passport.use(
      ...
      new OpenIDConnectStrategy(
        settings.config.get("auth.user.oidc"),
  ...
  // we are using named strategies since we have one for login and one for signup
  // by default, if there was no name, it would just be called 'local'
  if (settings.config.get("auth.admin.method").includes("ldap")) {
    passport.use(
      "admin-ldap",
      new LDAPStrategy(
        settings.config.get("auth.admin.ldap"),
        (req, user, done) => {
          models.admin
            .findAll({
              where: {
            ...
            .then(function (users) {
              ...
              if (!users || users.length == 0) {
                ...
                var dbUser = users.find((dbUser) => {
                  return (
                    dbUser.logon_id.toLowerCase() === user.cn.toLowerCase()
                  );
                });
                if (dbUser) {
                  dbUser.lastLogin = moment();
                  ...
                  req.flash(
                    "loginMessage",
                    "LDAP Benutzer*in nicht in Liste der Admin-Accounts"
                  );
                  done(null, false);
                }
              }
  ...
  if (settings.config.get("auth.admin.method").includes("oidc")) {
    passport.use(
      "admin-oidc",
      new OpenIDConnectStrategy(
        settings.config.get("auth.admin.oidc"),
        (req, user, done) => {
          models.admin
            .findAll({
              where: {
            ...
            .then(function (users) {
              // if there are no LDAP users, all OIDC users are accepted
              if (!users || users.length == 0) {
                ...
                var dbUser = users.find((dbUser) => {
                  return (
                    dbUser.email.toLowerCase() === user.username.toLowerCase()
                  );
                });
                if (dbUser) {
                  dbUser.lastLogin = moment();
                  ...
                  return done(
                    ...
                    req.flash(
                      "loginMessage",
                      "OIDC Benutzer*in nicht in Liste der Admin-Accounts"
                    )
                  ); // req.flash is the way to set flashdata using connect-flash
                }
              }
          ...
          .then(function (user) {
            ...
            if (!user.comparePassword(password)) {
              ...
              return done(
                null,
                false,
                req.flash("loginMessage", "Falsches Passwort")
            ...
            models.admin
              ...
              .catch((error) => {
                  ...
                  null,
                  false,
                  req.flash("loginMessage", "Fehler: " + error)
                ); // create the loginMessage and save it to session as flashdata
              });
          });
      }
