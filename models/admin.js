...
module.exports = (sequelize, DataTypes) => {
	...
	Admin.findByToken = function (token) {
		return Admin.findOne({where: { passwordResetToken: token }})
			.then(user => {
	...
	Admin.emailAddressTaken = function (email) {
		return Admin.count({where: { email: email }})
			.then(count => {
				return count !== 0;
			});
	};

	Admin.prototype.comparePassword = function comparePassword(candidatePassword, cb) {
