...
module.exports = function(app){
	...
	router.post('/contract/import', security.isLoggedInAdmin, multer().none(), function(req, res, next) {
			...
			return Promise.join(models.user.findByPk(getValue('contract_user_id')),
				...
				(user, contractId) => {
					...
					if (req.body.termination_type == "T") {
						termination_date = !!getValue('contract_termination_date', req.body.termination_date_T)?moment(req.body.termination_date_T):null;
					...
					var interest_payment_type = getValue('contract_interest_payment_type', req.body.interest_payment_type);
					['end', 'yearly'].forEach(type => {
						if (!!interest_payment_type && interest_payment_type.toLowerCase() === _t('interest_payment_type_' + type).toLowerCase()) {
					...
					return models.contract.create({
							id: contractId,
							sign_date: moment(getValue('contract_sign_date', req.body.sign_date)),
						...
						.then(contract => {
							if (depositAmount > 0) {
								...
								return models.transaction.create(transaction, { trackOptions: utils.getTrackOptions(req.user, true) })
									.then(transaction => {return contract});
		...
		utils.processImportFile(req.body.import_file_id, 'contract', JSON.parse(req.body.import_mappings), validateAndCreate)
			.then(result => {
				var errors = result.filter(entry => { return !entry.success; });				
				var contractIds = result.filter(entry => { return entry.success; }).map(entry => { return entry.object.id });
				return models.user.findFetchFull(models, {}, (user, contract) => {
					...
					.then(users => {				
						...
						var contractTable = contracttable.generateContractTable(req, res, users).setColumnsVisible(visibleColumns);
						res.render('process/import_result', {rowCount: result.length, errors : errors, contracts: contractTable, importTarget: 'contract'});
					})
	...
	router.post('/contract/bulkdelete', security.isLoggedInAdmin, multer().none(), function(req, res, next) {
		var ids = JSON.parse(req.body.ids);
		models.contract.destroy({ where: { id: ids }, trackOptions: utils.getTrackOptions(req.user, true)})
			.then(function(deleted) {
				if(deleted > 0) {
				 	res.json({deletedRows: deleted});
				} else {
					res.json({error: 'Kreditverträge konnten nicht gelöscht werden:' + error});
				}
